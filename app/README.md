...
基于 **Expo SDK 54** 的 React Native 客户端：底部 Tab 含 **日程 / 财务 / 用户**，日程栈内含列表与通知收件箱；**主题与 Manrope 字体**、`i18next` 文案、`AsyncStorage` 日程仓储、**WebView** 加载本地/Vite 前端（配置见 `core/config`）。开发时使用仓库根目录 `pnpm dev:app` 或在 `app/` 下 `pnpm start`。
